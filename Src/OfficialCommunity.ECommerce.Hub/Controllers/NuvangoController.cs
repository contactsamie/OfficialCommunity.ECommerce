...
namespace OfficialCommunity.ECommerce.Hub.Controllers
{
    ...
    public class NuvangoController : Controller
    {
        ...
        public async Task<ActionResult> ProductCount()
        {
            ...
            using (_logger.BeginScope(entry))
            {
                try
                {
                    ...
                    return Json(Mapper.Map<CommonService.GetProductsCountResponse, ViewableProductCount>(count.Response));
                }
                catch (Exception e)
                {
        ...
        [Authorize]
        public async Task<ActionResult> ProductVariants([DataSourceRequest]DataSourceRequest request)
        {
            var passport = Passport.Generate();
